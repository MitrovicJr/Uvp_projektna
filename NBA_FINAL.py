...
response = requests.get("https://stats.nba.com/stats/leagueLeaders?LeagueID=00&PerMode=Totals&Scope=S&Season=2022-23&SeasonType=Regular+Season&StatCategory=PTS")
data = response.json()
...
if player is not None:
    ...
    for idx, similar_player in enumerate(similar_players[:3], 1):
        print(f"Player {idx}:")
    ...
    plt.plot(graph_minutes, regression_line, color='red', label='Regression Line')
